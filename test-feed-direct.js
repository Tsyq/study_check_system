async function testFeedAPIDirectly() {
  console.log('🔍 直接测试关注动态API...\n');
  ...
  try {
    ...
    if (loginResponse.status === 200) {
      ...
      console.log(`API响应状态: ${feedResponse.status}`);
      ...
      if (feedResponse.status === 200) {
        ...
        if (feedData.checkins.length > 0) {
          console.log('  - 动态内容:');
          feedData.checkins.forEach((checkin, index) => {
            console.log(`    ${index + 1}. ${checkin.user.username}: ${checkin.content}`);
